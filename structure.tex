...
\usepackage[margin=1.6cm, top=1.2cm,right=1.9cm,left=1.9cm,
            footskip=1cm, headheight=31pt, headsep=0.2cm]{geometry}
%\geometry{showframe}
...
\definecolor{grn}{RGB}{76, 153, 76}          % зелёный (для определений)
\definecolor{chapclr}{RGB}{200, 60, 60}      % красный (для заголовков глав/билетов)
...
\makeatletter
\newif\ifinappendix
\pretocmd{\appendix}{\global\inappendixtrue}{}{}
\makeatother
...
\newtheorem{consnT}{Следствие}[chapter] %штрихованное.
  ...
  {\noindent
    \if@mainmatter
      \begin{tikzpicture}[remember picture]
        \node (n) at (\Gm@lmargin,-\Gm@tmargin)
          [inner sep=3mm, text width={\textwidth}, text opacity=1,
           font=\huge\sffamily\bfseries, text=black]
          {\thechapter. #1};
        \begin{scope}[on background layer]
          \draw [line width=2pt, rounded corners=14pt, prpl, fill=prpl!20]
            (n.north -| current page.east) -| (n.south west)
            -- (n.south -| current page.east);
        \end{scope}
      \end{tikzpicture}%
    \else
      \begin{tikzpicture}[remember picture]
        \node (n) at (\Gm@lmargin,-\Gm@tmargin)
          [inner sep=3mm, text width={\textwidth-0mm}, text opacity=1,
           font=\huge\sffamily\bfseries, text=black]
          {#1};
        \begin{scope}[on background layer]
          \draw [line width=2pt, rounded corners=14pt, chapclr, fill=prpl!20]
            (n.north -| current page.east) -| (n.south west)
            -- (n.south -| current page.east);
        \end{scope}
      \end{tikzpicture}%
